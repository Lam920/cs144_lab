...
/*Delete NAT entry in Mapping table if it is not used*/
void *sr_nat_timeout(void *nat_ptr) {  /* Periodic Timout handling */
  ...
  while (1) {
    ...
    while(entry)
    {
      ...
      if( (difftime(curtime,entry->last_updated) > 60.0 && entry->type == (sr_nat_mapping_type)nat_mapping_icmp) | (difftime(curtime,entry->last_updated) > 300.0 && entry->type == nat_mapping_tcp))
      {
